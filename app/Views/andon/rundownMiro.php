        ...
        <thead id="fixed-div">
            <tr>
                ...
                <th colspan="6" rowspan="3" style="color: white;">
                    <h1>MIRO ACHIEVEMENT & PAYMENT DUE</h1>
...
<script>
    $(function() {
        ...
        function fetchAndRenderInvoices() {
            $.ajax({
                url: '<?= base_url("rundownMiro/getMiro") ?>',
                method: 'GET',
                dataType: 'json',
                success: function(data) {
                    allData = data;
                    data.forEach(function(item) {
                        var generatedDate = new Date(item.date_miro);
                        ...
                        var dateReceived = new Date(item.date_approve)
                        var differenceInTime = generatedDate - dateReceived;
	...
	function hitungHariKerja(startDate, endDate) {
        ...
        function renderInvoices(data) {
            ...
            for (var key in groupedData) {
                ...
                group.forEach(function(value, index) {
                    var hariKerja = hitungHariKerja(value.date_approve, value.date_miro);
                    var row = `<tr>`
		    console.log(value.date_approve)
                    if (index === 0) {
                        row += `<td rowspan="${rowspan}">${value.company_name !== null ? value.company_name : "-"}</td>`;
                    }
                    if (index === 0) {
                        row += `<td rowspan="${rowspan}">${value.dropbox_id !== null ? value.dropbox_id : "-"}</td>`;
                    }
                    row += `<td>${value.no_invoice !== null ? value.no_invoice : "-"}</td>
                            <td>${value.date_approve !== null ? formatDate(value.date_approve)  : "-"}</td>
                            <td>${value.date_dok_ok !== null ? formatDate(value.date_dok_ok) : "-"}</td>
                            <td>${value.date_in_pud !== null ? formatDate(value.date_in_pud) : "-"}</td>
                            <td>${value.date_out_pud !== null ? formatDate(value.date_out_pud) : "-"}</td>
                            <td>${value.date_miro !== null ? formatDate(value.date_miro) : "-"}</td>
                            <td>${value.date_approve !== null ? hariKerja : "-"}</td>
                        </tr>`;
                    tbody.append(row);
                });
            }
        ...
        function pageScroll() {
            ...
            if (!$('#date_filter').is(':focus') && $('.ui-datepicker').css('display') === 'none') {
        ...
        // setInterval(fetchAndRenderInvoices, 5000);
    });
</script>
