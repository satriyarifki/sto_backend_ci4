            ...
            <div class="card-body">
                ...
                <div class="col-auto">
                    <div class="form-group">
                        <div class="input-group">
                            <div class="input-group-append">
                                <span class="input-group-text">Nomor GR</span>
                            </div>
                            <input type="text" id="nogr" class="form-control" />
                        </div>
                    </div>
                </div>
                <div class="col-auto">
...
$(document).ready(function(){
    ...
    function updateTableContent() {
        ...
        var noGR = document.getElementById('nogr').value;
        var vendor = document.getElementById('vendorSelect').value;

        if ((startDate || endDate || poNumber || noGR) && vendor){
            $('#example').DataTable().processing(true);
    
            var xhr = new XMLHttpRequest();
            xhr.open('POST', 'track_maj_real', true);
            ...
            var data = JSON.stringify({startDate: startDate, endDate: endDate, poNumber: poNumber, noGR: noGR, vendor: vendor});
            xhr.send(data);
        } else {
            toastr.info('Masukan input tanggal atau nomor PO terlebih dahulu serta pilih Vendor nya')
        }
            ...
            $.ajax({
                ...
                success: function(response) {
                                ...
                                if (response.result.dok_ok == "Y") {
                                    if (response.result.delete_pud == "Y"){
                                        ...
                                        if (response.result.status_miro == "Y"){
                                            ...
                                            var dateMiro = new Date(response.result.date_miro);
                                            var dayMiro = dateMiro.getDate();
                                            var monthMiro = namaBulan[dateMiro.getMonth()];
                                            var yearMiro = dateMiro.getFullYear();
                                            topStart = `${dayMiro} ${monthMiro} ${yearMiro}`;
                                        } else {
                                            approve = 'Verifiying';
                    ...
                    table.row.add([
                        ...
                        '<button class="btn btn-info">' + topStart + '</button>',
                        value.no_invoice,
                        value.no_po,
                        value.no_gr,
                        tanggalAkhir,
