...
class M_invoicing_npkp extends Model
{
    protected $table = 'invoice_npkp';
    public $running_id_npkp = "NPKP";
    ...
    public function get_running_id_npkp()
    {
        $uniqid_record = $this->db->table('uniq_id_buffer')
                        ->select('Uniqid, Date')
                        ->where('Name', 'invoice_npkp')
                        ->get()
                        ->getRowArray();
        ...
        if ($uniqid_record) {
            ...
            $next_invoice_id = $this->running_id_npkp . $today_formatted . $next_id;
    ...
    public function insert_invoice_npkp($data)
    {
        return $this->db->table('invoice_npkp')->insert($data);
    }

    public function insert_arsip_pdf_npkp($data)
    {
        return $this->db->table('arsip_pdf_npkp')->insert($data);
    }
    ...
    {
        $builder = $this->db->table('invoice_npkp')
			->select('invoicing_id,
