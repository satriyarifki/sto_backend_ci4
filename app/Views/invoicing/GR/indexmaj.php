...
$(document).ready(function(){
    ...
    function updateTableContent() {
        ...
        if ((startDate || endDate || poNumber) && vendor){
            $('#example').DataTable().processing(true);
    
            var xhr = new XMLHttpRequest();
            xhr.open('POST', 'realmaj', true);
            ...
            var data = JSON.stringify({startDate: startDate, endDate: endDate, poNumber: poNumber, vendor: vendor});
            xhr.send(data);
        } else {
    ...
    document.getElementById('exportData').addEventListener('click', function() {
        ...
        xhr.onreadystatechange = function() {
            if (xhr.readyState === 4 && xhr.status === 200) {
                var data = JSON.parse(xhr.responseText);
                if (data.hasOwnProperty('data_sap')) {
                    var headers = ["No.", "No. Item", "Status", "No. PO", "No. GR", "Surat Jalan", "Part Number", "Part Name", "Qty", "Tanggal GR"];
                    var tableData = [];
